...
public interface ITrinhDoDaoTaoRepository
{
    Task<(IEnumerable<TrinhDoDaoTao>?, int)> FilterAsync(TrinhDoDaoTaoFilter model);
    Task<TrinhDoDaoTao> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(TrinhDoDaoTaoDto model, long createdBy);
    Task UpdateAsync(long id, TrinhDoDaoTaoDto model, long updatedBy);
    Task DeleteAsync(long id, long deletedBy);
}

public class TrinhDoDaoTaoRepository : ITrinhDoDaoTaoRepository
{
    ...
    private const string Label = "Trình độ đào tạo";

    public TrinhDoDaoTaoRepository(
        IMapper mapper,
    ...
    public async Task<(IEnumerable<TrinhDoDaoTao>?, int)> FilterAsync(TrinhDoDaoTaoFilter model)
    {
        var query = _educationLevelRepository
            .Select();
    ...
    public async Task<TrinhDoDaoTao> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _educationLevelRepository
            .Select(isTracking);
    ...
    public async Task CreateAsync(TrinhDoDaoTaoDto model, long createdBy)
    {
        var query = _educationLevelRepository
            .Select();
        ...
        var newItem = _mapper.Map<TrinhDoDaoTao>(model);
        newItem.CreatedAt = DateTime.UtcNow;
        newItem.UpdatedAt = DateTime.UtcNow;
        _educationLevelRepository.Insert(newItem);
    ...
    public async Task UpdateAsync(long id, TrinhDoDaoTaoDto model, long updatedBy)
    {
        var item = await GetByIdAsync(id, true);
        var isExist = await _educationLevelRepository
            .Select()
            .Where(p => p.Id != id)
            .FirstOrDefaultAsync(p =>
                p.Name.ToLower().ToLower() == model.Name.ToLower() ||
        ...
        _educationLevelRepository.Update(item);
        await _educationLevelRepository.SaveChangesAsync();
    ...
    {
        var item = await GetByIdAsync(id, true);
        _educationLevelRepository.Delete(item);
        await _educationLevelRepository.SaveChangesAsync();
