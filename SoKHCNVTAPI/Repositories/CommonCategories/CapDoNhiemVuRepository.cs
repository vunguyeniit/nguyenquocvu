...
public interface ICapDoNhiemVuRepository
{
    Task<(IEnumerable<CapDoNhiemVu>?, int)> FilterAsync(MissionLevelFilter model);
    Task<CapDoNhiemVu> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(MissionLevelDto model, long createdBy);
    Task UpdateAsync(long id, MissionLevelDto model, long updatedBy);
    Task DeleteAsync(long id, long deletedBy);
}

public class CapDoNhiemVuRepository : ICapDoNhiemVuRepository
{
    private readonly IMapper _mapper;
    private readonly IRepository<CapDoNhiemVu> _degreeRepository;
    //private readonly IRepository<User> _userRepository;
    private readonly IActivityLogRepository _activityLogRepository;
    //private readonly IMemoryCachingService _cachingService;

    private const string Label = "Cấp độ nhiệm vụ";

    public CapDoNhiemVuRepository(
        IMapper mapper,
        //IMemoryCachingService cachingService,
        IRepository<CapDoNhiemVu> degreeRepository,
    ...
    public async Task<(IEnumerable<CapDoNhiemVu>?, int)> FilterAsync(MissionLevelFilter model)
    {
    ...
    public async Task<CapDoNhiemVu> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _degreeRepository
            .Select(isTracking);
    ...
    public async Task CreateAsync(MissionLevelDto model, long createdBy)
    {
        var query = _degreeRepository
            .Select();
        ...
        var newItem = _mapper.Map<CapDoNhiemVu>(model);
    ...
    public async Task UpdateAsync(long id, MissionLevelDto model, long updatedBy)
    {
