...
{
    Task<(IEnumerable<Title>?, int)> FilterAsync(TitleFilter model);
    Task<Title> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(TitleDto model, long createdBy);
    Task UpdateAsync(long id, TitleDto model, long updatedBy);
...
public class TitleRepository : ITitleRepository
{
    ...
    private const string Label = "Chức danh";
    ...
    public async Task<(IEnumerable<Title>?, int)> FilterAsync(TitleFilter model)
    {
        var query = _titleRepository
            .Select();
    ...
    public async Task<Title> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _titleRepository
            .Select(isTracking);
    ...
    public async Task CreateAsync(TitleDto model, long createdBy)
    {
        var query = _titleRepository
            .Select();
        ...
        var newItem = _mapper.Map<Title>(model);
        newItem.CreatedAt = DateTime.UtcNow;
        newItem.UpdatedAt = DateTime.UtcNow;
        _titleRepository.Insert(newItem);
        await _titleRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"chức danh với mã #{newItem.Code} tên: {newItem.Name} thành công.",
    ...
    public async Task UpdateAsync(long id, TitleDto model, long updatedBy)
    {
        var item = await GetByIdAsync(id, true);
        var isExist = await _titleRepository
            .Select()
            .Where(p => p.Id != id)
            .FirstOrDefaultAsync(p =>
                p.Name.ToLower().ToLower() == model.Name.ToLower() ||
        ...
        _titleRepository.Update(item);
        await _titleRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"chức danh với mã #{item.Code} tên: {item.Name} thành công.",
