...
public interface IDinhDanhToChucRepository
{
    Task<(IEnumerable<DinhDanhToChuc>?, int)> FilterAsync(OrganizationIdentifierFilter model);
    Task<DinhDanhToChuc> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(OrganizationIdentifierDto model, long createdBy);
    Task UpdateAsync(long id, OrganizationIdentifierDto model, long updatedBy);
    Task DeleteAsync(long id, long deletedBy);
}

public class DinhDanhToChucRepository : IDinhDanhToChucRepository
{
    ...
    private const string Label = "Mã định danh tổ chức";

    public DinhDanhToChucRepository(
        IMapper mapper,
    ...
    public async Task<(IEnumerable<DinhDanhToChuc>?, int)> FilterAsync(OrganizationIdentifierFilter model)
    {
        var query = _organizationIdentifierRepository
            .Select();
    ...
    public async Task<DinhDanhToChuc> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _organizationIdentifierRepository
            .Select(isTracking);
    ...
    public async Task CreateAsync(OrganizationIdentifierDto model, long createdBy)
    {
        var query = _organizationIdentifierRepository
            .Select();
        ...
        var newItem = _mapper.Map<DinhDanhToChuc>(model);
        newItem.CreatedAt = DateTime.UtcNow;
        newItem.UpdatedAt = DateTime.UtcNow;
        _organizationIdentifierRepository.Insert(newItem);
        await _organizationIdentifierRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"mã định danh tổ chức với mã #{newItem.Code} tên: {newItem.Name} thành công.",
    ...
    public async Task UpdateAsync(long id, OrganizationIdentifierDto model, long updatedBy)
    {
        var item = await GetByIdAsync(id, true);
        var isExist = await _organizationIdentifierRepository
            .Select()
            .Where(p => p.Id != id)
            .FirstOrDefaultAsync(p =>
                p.Name.ToLower().ToLower() == model.Name.ToLower() ||
        ...
        _organizationIdentifierRepository.Update(item);
        await _organizationIdentifierRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"mã định danh tổ chức với mã #{item.Code} tên: {item.Name} thành công.",
    ...
    {
        var item = await GetByIdAsync(id, true);
        _organizationIdentifierRepository.Delete(item);
        await _organizationIdentifierRepository.SaveChangesAsync();
