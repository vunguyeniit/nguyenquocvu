...
{
    Task<(IEnumerable<Degree>?, int)> FilterAsync(DegreeFilter model);
    Task<Degree> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(DegreeDto model, long createdBy);
    Task UpdateAsync(long id, DegreeDto model, long updatedBy);
...
public class HocViRepository : IHocViRepository
{
    ...
    private const string Label = "Học vị";
    ...
    public async Task<(IEnumerable<Degree>?, int)> FilterAsync(DegreeFilter model)
    {
    ...
    public async Task<Degree> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _degreeRepository.Select(isTracking);
        var item = await query.SingleOrDefaultAsync(p => p.Id == id);
        if (item == null) throw new ArgumentException($"Không tìm thấy {Label}!");
        return item;
    }

    public async Task CreateAsync(DegreeDto model, long createdBy)
    {
        var query = _degreeRepository
            .Select();
        ...
        var newItem = _mapper.Map<Degree>(model);
        ...
        {
            Contents = $"học vị với mã #{newItem.Code} và tên: {newItem.Name} thành công.",
            Params = newItem.Code,
            Target = "Degree",
    ...
    public async Task UpdateAsync(long id, DegreeDto model, long updatedBy)
    {
        ...
        var log = new ActivityLogDto
        {
            Contents = $"học vị với mã #{item.Code} và tên: {item.Name} thành công.",
            Params = item.Code,
            Target = "Degree",
