...
public interface ILoaiDuAnRepository
{
    Task<(IEnumerable<LoaiDuAn>?, int)> FilterAsync(LoaiDuAnFilter model);
    Task<LoaiDuAn> GetByIdAsync(long id, bool isTracking = false);
    Task CreateAsync(LoaiDuAnDto model, long createdBy);
    Task UpdateAsync(long id, LoaiDuAnDto model, long updatedBy);
    Task DeleteAsync(long id, long deletedBy);
}

public class LoaiDuAnRepository : ILoaiDuAnRepository
{
    private readonly IMapper _mapper;
    private readonly IRepository<LoaiDuAn> _projectTypeRepository;
    private readonly IRepository<User> _userRepository;
    private readonly IActivityLogRepository _activityLogRepository;
    private readonly IMemoryCachingService _cachingService;

    private const string Label = "Loại dự án";

    public LoaiDuAnRepository(
        IMapper mapper,
    ...
    public async Task<(IEnumerable<LoaiDuAn>?, int)> FilterAsync(LoaiDuAnFilter model)
    {
        var query = _projectTypeRepository
            .Select();
    ...
    public async Task<LoaiDuAn> GetByIdAsync(long id, bool isTracking = false)
    {
        var query = _projectTypeRepository
            .Select(isTracking);
    ...
    public async Task CreateAsync(LoaiDuAnDto model, long createdBy)
    {
        var query = _projectTypeRepository
            .Select();
        ...
        var newItem = _mapper.Map<LoaiDuAn>(model);
        newItem.CreatedAt = DateTime.UtcNow;
        newItem.UpdatedAt = DateTime.UtcNow;
        _projectTypeRepository.Insert(newItem);
        await _projectTypeRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"loại hình dự án với mã #{newItem.Code} tên: {newItem.Name} thành công.",
    ...
    public async Task UpdateAsync(long id, LoaiDuAnDto model, long updatedBy)
    {
        var item = await GetByIdAsync(id, true);
        var isExist = await _projectTypeRepository
            .Select()
            .Where(p => p.Id != id)
            .FirstOrDefaultAsync(p =>
                p.Name.ToLower().ToLower() == model.Name.ToLower() ||
        ...
        _projectTypeRepository.Update(item);
        await _projectTypeRepository.SaveChangesAsync();

        // ____________ Log ____________
        var log = new ActivityLogDto
        {
            Contents = $"loại hình dự án với mã #{item.Code} tên: {item.Name} thành công.",
    ...
    {
        var item = await GetByIdAsync(id, true);
        _projectTypeRepository.Delete(item);
        await _projectTypeRepository.SaveChangesAsync();
