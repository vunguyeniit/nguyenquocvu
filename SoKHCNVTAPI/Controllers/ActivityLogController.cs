...
public class ActivityLogController : BaseController
{
    ...
    public ActivityLogController(IActivityLogRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository)
    { _repository = repository; }


    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] ActivityLogFilter model)
    {
        if (!await Can("Xem tài khoản", "Tài khoản"))
        {
