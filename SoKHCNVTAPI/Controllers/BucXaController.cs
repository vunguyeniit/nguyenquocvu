...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/bucxa")]
[ApiVersion("1")]
public class BucXaController : BaseController
{
    private readonly IBucXaRepository _repo;

    public BucXaController(IBucXaRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetBucXas([FromQuery] BucXaFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> TaoBucXa([FromBody] BucXaDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> CapNhatBucXa(long id, [FromBody] BucXaDto model)
    {
    ...
    [HttpDelete("{id:long}")]
    public async Task<IActionResult> DeleteBucXa(long id)
    {
