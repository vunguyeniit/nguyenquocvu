...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/sohuutritue")]
[ApiVersion("1")]
public class SoHuuTriTueController : BaseController
{
    private readonly ISoHuuTriTueRepository _repo;
    string module = "Sỡ hữu trí tuệ";
    public SoHuuTriTueController(ISoHuuTriTueRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetSHTT([FromQuery] SoHuuTriTueFilter model)
    {
       // if (!await Can("Xem sở hữu trí tuệ", "Sỡ hữu trí tuệ")) return PermissionMessage();
    ...
    [HttpPost]
    public async Task<IActionResult> TaoSHTT([FromBody] SoHuuTriTueDto model)
    {
        if (!await Can("Thêm sở hữu trí tuệ", module)) return PermissionMessage();
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> CapNhatChuyenGia(long id, [FromBody] SoHuuTriTueDto model)
    {
        if (!await Can("Cập nhật sở hữu trí tuệ", module)) return PermissionMessage();
        if (id <= 0)
        {
            return StatusCode(StatusCodes.Status200OK, new BaseResponse
            {
    ...
    {
        if (!await Can("Xóa sở hữu trí tuệ", module)) return PermissionMessage();
        if (id <= 0)
        {
            return StatusCode(StatusCodes.Status200OK, new BaseResponse
            {
