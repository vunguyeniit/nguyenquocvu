...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/lgsp")]
[ApiVersion("1")]
public class LGSPController : BaseController
{
    private readonly ILGSPRepository _repo;

    public LGSPController(ILGSPRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetLGSPs([FromQuery] LGSPFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> TaoLGSP([FromBody] LGSPDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> CapNhatLGSP(long id, [FromBody] LGSPDto model)
    {
        if (id <= 0)
        {
            return StatusCode(StatusCodes.Status200OK, new BaseResponse
    ...
    [HttpDelete("{id:long}")]
    public async Task<IActionResult> DeleteLGSP(long id)
    {
        if (id <= 0)
        {
            return StatusCode(StatusCodes.Status200OK, new BaseResponse
