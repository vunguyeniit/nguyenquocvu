...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/vaitrothamgia")]
[ApiVersion("1")]
public class VaiTroThamGiaController : BaseController
{
    private readonly IVaiTroThamGiaRepository _repo;

    public VaiTroThamGiaController(IVaiTroThamGiaRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] VaiTroThamGiaFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] VaiTroThamGiaDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] VaiTroThamGiaDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
