...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/mission-identifiers")]
[ApiVersion("1")]
public class DinhDanhNhiemVuController : BaseController
{
    private readonly IDinhDanhNhiemVuRepository _repo;

    public DinhDanhNhiemVuController(IDinhDanhNhiemVuRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] MissionIdentifierFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] MissionIdentifierDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] MissionIdentifierDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
