...
public class DepartmentController : BaseController
{
    ...
    public DepartmentController(IDonViRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] DonViFilter model)
    {
        var (items, records) = await _repo.FilterAsync(model);
        return StatusCode(StatusCodes.Status200OK, new PaginationBaseResponse
        {
            Message = "Truy xuất danh sách phòng ban thành công!",
    ...
    public async Task<IActionResult> GetAsync(long id)
    {
        ...
        {
            Message = "Truy xuất phòng ban thành công!",
            Data = item
        });
    }
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] DonViDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] DonViDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
