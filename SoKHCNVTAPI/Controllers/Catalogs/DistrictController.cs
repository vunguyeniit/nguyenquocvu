...
public class DistrictController : BaseController
{
    ...
    public DistrictController(IDistrictRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] long provinceId)
    {
        var items = await _repo.FilterAsync(provinceId);
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] DistrictDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] DistrictDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
