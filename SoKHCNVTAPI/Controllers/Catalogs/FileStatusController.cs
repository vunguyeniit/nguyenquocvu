...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/file-statuses")]
[ApiVersion("1")]
public class FileStatusController : BaseController
{
    private readonly IFileStatusRepository _repo;

    public FileStatusController(IFileStatusRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] FileStatusFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] FileStatusDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] FileStatusDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
