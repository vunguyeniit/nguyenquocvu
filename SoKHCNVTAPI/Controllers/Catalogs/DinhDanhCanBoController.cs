...
[ApiController]
[Authorize]
[Route("/api/v{version:apiVersion}/officer-identifiers")]
[ApiVersion("1")]
public class DinhDanhCanBoController : BaseController
{
    private readonly IDinhDanhCanBoRepository _repo;

    public DinhDanhCanBoController(IDinhDanhCanBoRepository repository, IRepository<Permission> permissionRepository, IUserRepository userRepository,
        IRepository<Role> rolePermission) : base(permissionRepository, rolePermission, userRepository) { _repo = repository; }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] DinhDanhCanBoFilter model)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] DinhDanhCanBoDto model)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Edit(long id, [FromBody] DinhDanhCanBoDto model)
    {
        var userId = long.Parse(HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        if (id <= 0)
        {
